...
export function FundCard({
  ...
  }, [depositReceipt.status, onSuccess]);
  ...
  const pendingMessage = useMemo(() => {
    ...
    if (isDepositPending && depositHash) {
      return `Processing deposit… Tx: ${depositHash.slice(0, 10)}…`;
    }
    return "";
  }, [isApprovalPending, approvalHash, isDepositPending, depositHash]);

  return (
    <Card className="w-full shadow-lg sm:max-w-md md:max-w-md">
      <CardContent className="space-y-6 px-6 pb-2">
        {showSteps ? <FlowSteps current={2} /> : null}
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="space-y-1">
            <p className="text-xs გენ font-semibold uppercase text-muted-foreground">Flow</p>
        ...
        <div className="space-y-2">
          <Label className="text-xs font-semibold uppercase text-muted-foreground">Sepolia PYUSD token</Label>
        ...
        <div className="space-y-2">
          <Label className="text-xs font-semibold uppercase text-muted-foreground">Central wallet contract</Label>
          <div className="flex items-center gap-3 rounded-[var(--radius-lg)] border bg-muted px-4 py-3">
            <div className="flex flex-1 flex-col">
              <span className="font-mono text-sm text-card-foreground">{treasuryAddressMasked}</span>
              <span className="text-[11px] text-muted-foreground">Funds land here after you deposit</span>
            </div>
            <Button variant="outline" size="sm" onClick={() => handleCopy(CENTRAL_WALLET_ADDRESS, "Treasury address copied")}
            >
              Copy
            </Button>
          </div>
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs font-semibold text-primary">Wallet balance: {walletBalance.toFixed(2)} PYUSD</p>
        </div>
      ...
      <CardFooter className="flex flex-col gap-4 px-6 pb-6">
        <div className="flex w-full flex-col gap-3">
          ...
          >
            {approving || isApprovalPending ? "Approving…" : "Approve"}
          </Button>
          <Button
            size="lg"
            className="w-full rounded-full"
          ...
          >
            {depositing || isDepositPending ? "Depositing…" : "Deposit"}
          </Button>
        </div>
        {pendingMessage ? <p className="text-center text-[11px] text-muted-foreground">{pendingMessage}</p> : null}
        {feedback ? <p className="text-center text-[11px] text-muted-foreground">{feedback}</p> : null}
        {approvalError ? <p className="text-center text-[11px] text-destructive">{approvalError}</p> : null}
        {depositError ? <p className="text-center text-[11px] text-destructive">{depositError}</p> : null}
      </CardFooter>
    </Card>
  );
}
